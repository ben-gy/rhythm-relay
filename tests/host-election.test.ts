...
describe('host election — nobody hosts a mesh that has not formed', () => {
  ...
  it('falls back to an election if the room turns out to have no host', async () => {
    ...
    //
    // The wait to hear from an incumbent is now SETTLE_MS (6s), up from 2.5s.
    // The old window was short enough that a joiner on a slow handshake gave up
    // and self-elected while a perfectly live host sat one second away — which
    // is how a joiner ended up stealing a room that already had an owner.
